({
    ...
    setRedirect : function(component, client_id, RedirectURI) {
        window.location.href = 'https://www.dropbox.com/oauth2/authorize?response_type=code&client_id=' + client_id + '&redirect_uri=' + RedirectURI + '&state='+component.get("v.recordId");        
    },
    ...
    upload: function(component, file, fileContents, file_path) {
        ...
        this.uploadChunk(component, file, fileContents, fromPos, toPos, '', 0, false, file_path, 'appendStart');   
    },
     
    uploadChunk : function(component, file, fileContents, fromPos, toPos, uploadId, offset, commitFlag, file_path, uploadStatus) {
        ...
        action.setCallback(this, function(a) {
            ...
            if(a.getReturnValue() != ''){
                ...
                if (fromPos < toPos) {
                    console.log("upload");
                    self.uploadChunk(component, file, fileContents, fromPos, toPos, uploadId, offset, false, file_path, 'appendUpload');  
                }else{            
                    console.log("commit");
                    self.uploadChunk(component, file, fileContents, fromPos, toPos, uploadId, offset, true, file_path, 'appendFinish');
